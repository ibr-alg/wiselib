...
namespace wiselib
{
   ...
   extern "C" int construct( Component *comp )
   {
      ...
      if ((nerror = oc_regInterfaces(comp, 5, "IMXRadio", &ir, sizeof(IMXRadio),
      														"IHWControl", &ihc, sizeof(IHWControl),
      														"ILife", &il, sizeof(ILife),
      														"ITimerCallback", &itc, sizeof(ITimerCallback),
      														"IMXRadioCallback", &irc, sizeof(IMXRadioCallback))) != OK)
      														return nerror;
      ...
      if ((nerror = oc_regReceptacles(comp, 3,
                        "IMXRadio", &st -> radio, sizeof(IMXRadio),
                        "IHWControl", &st -> hardwareControl, sizeof(IHWControl),
                        "ITimer", &st -> timer, sizeof(ITimer))) != OK)
      {
         return nerror;
      }
