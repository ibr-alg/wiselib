...
#	times=$2
	...
	if [ -n $ans ] || [ $ans != "n" ]
	then
		echo -en "#Total\theads\tsimple\trounds\ttime\tcl.diam\tavgsize\tmsg\tJOIN\tACC\tDENY\tRESUME\tCHD\tJD\tIT\t\\" > data/"$d_parameter"."$topologies"."$algo_name".dat
		...
		do
			...
			END	{
				...
				chd_mess=0;
				jd_mess=msg_join+msg_jacc+msg_jdeny;
				it_mess=msg_resume;
				print total_nodes "\t" heads "\t" simple "\t" round_finished "\t" time_needed "\t" hops "\t" avg_size "\t" msg_ "\t" msg_join "\t" msg_jacc "\t" msg_jdeny "\t" msg_resume "\t" chd_rounds "\t" jd_rounds "\t" it_rounds "\t" chd_mess "\t" jd_mess "\t" it_mess "\t\\\\";
