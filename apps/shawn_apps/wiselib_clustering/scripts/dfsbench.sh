...
#	times=$2
	...
	if [ -n $ans ] || [ $ans != "n" ]
	then
		echo -en "#Total\theads\tsimple\trounds\ttime\tcl.diam\tavgsize\tmsg\tDISC\tREPLY\tJ_REQ\tJ_REJ\tRESUME\t\\" > data/"$d_parameter"."$topologies"."$algo_name".dat
		...
		do
			...
			END	{
				...
				hops=hops/simple;
				print total_nodes "\t" heads "\t" simple "\t" round_finished "\t" time_needed "\t" hops "\t" avg_size "\t" msg_ "\t" msg_disc "\t" msg_reply "\t" msg_jdeny "\t" msg_jreq "\t" msg_resume "\t\\\\";
