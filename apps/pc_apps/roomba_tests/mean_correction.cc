#include "stuff.h"
#include "corrected_mean_motion.h"
...
typedef wiselib::CorrectedMeanMotion<OsModel, Roomba> CorrectedMeanMotion;
CorrectedMeanMotion g_mean_motion;
 ...
 */
bool drive(CorrectedMeanMotion::Profile p, int distance = 0, int velocity = 0) {
 ...
 * @return false if the user cancelled, true otherwise
 */
bool turn(CorrectedMeanMotion::Profile p, int& cur_orientation,
  int velocity = 0, int angle = 0) {
  bool ok = false;
...
int main(int argc, char ** argv) {
  ...
  CorrectedMeanMotion::Profile ground_type;
  if(strcmp(argv[3], "iz250flur") == 0) {
    ground_type = CorrectedMeanMotion::LAMINATE;
  } else if(strcmp(argv[3], "seminarraum") == 0) {
    ground_type = CorrectedMeanMotion::CARPET;
  } else {
    cerr << "Unknown ground type. Currently only iz250flur and seminarraum "
      "are known." << endl;
    return -1;
  }
  ...
  if(strcmp(argv[1], "--turn") == 0 || strcmp(argv[1], "-t") == 0) {
    ...
    sprintf(g_log_file, "roomba-mean-turn-%d.log", int(time(NULL)));
    ...
    for(size_t v = 0; v < sizeof(velocities) / sizeof(int); v++) {
      for(size_t a = 0; a < sizeof(angles) / sizeof(int); a++) {
        if(!turn(ground_type, orientation, velocities[v], angles[a])) {
    ...
    while(ok = turn(ground_type, orientation))
      ; // until the user cancels
    return -1;

  } else if(strcmp(argv[1], "--drive") == 0 || strcmp(argv[1], "-d") == 0) {
    // time-based log file name
    sprintf(g_log_file, "roomba-mean-drive-%d.log", int(time(NULL)));
    ...
    for(size_t v = 0; v < sizeof(velocities) / sizeof(int); v++) {
      for(size_t a = 0; a < sizeof(distances) / sizeof(int); a++) {
        if(!drive(ground_type, distances[a], velocities[v])) {
          return -1; // user cancelled
        }
      }
    }
