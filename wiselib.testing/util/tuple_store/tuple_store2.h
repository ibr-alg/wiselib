...
namespace wiselib {
	...
	public:
		...
		typedef DataAVLTree<OsModel, Allocator, /*CompareLess<refcounted_data_t>,*/ Debug> data_tree_t;
		...
		public:
			...
			const data_t& operator[](size_t i) const { return nodes_[i]->data(); }
		...
		private:
			bool matches() {
				for(size_t i=0; i<tuple_.elements(); i++) {
					if(tuple_.nodes_[i]) {
						if(tuple_.nodes_[i] != (*list_iter_)->template as<Tuple>().nodes_[i]) {
							return false;
						}
					} // if
				} // for
		...
		int init(typename Allocator::self_pointer_t a, typename Debug::self_pointer_t debug = 0) {
			allocator_ = a;
			debug_ = debug;
			data_tree_.init(allocator_, &data_compare, debug_);
			tuple_tree_.init(allocator_, &tuple_compare, debug_);
			return SUCCESS;
		}
		...
		iterator insert(const T& t) {
			...
			tuple_tree_.insert(as_data(store_tuple));
		...
		query_iterator query_begin(const T& query, int index) {
			...
			else { return query_iterator(index_node->template as<const column_data_t>().begin(), query_tuple); }
		}
#endif // TUPLE_STORE_ENABLE_INDICES
		...
		int erase(iterator iter) {
			for(int i=0; i<N; i++) {
				typename data_tree_t::iterator diter = data_tree_.find(iter.template as<Tuple>().nodes_[i]->data());
	...
	private:
		...
		public:
			...
			int cmp(const column_data_t& other) const {
				return tuple_list_.front()->template as<Tuple>()[i].cmp(other.tuple_list_.front()->template as<Tuple>()[i]);
			}
...
#if TUPLE_STORE_ENABLE_INDICES
		typedef DataAVLTree<OsModel, Allocator, /*column_data_t,*/ /*CompareLess<column_data_t>,*/ Debug> index_tree_t;
		...
		void init_index(size_t column) {
			...
			idx.tree_->init(allocator_, &index_compare);
