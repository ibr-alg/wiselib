...
namespace wiselib {
	...
	public:
		...
		typedef AVLTree<OsModel_P, Allocator_P, Compare_P, Debug_P> self_type;
		typedef self_type* self_pointer_t;
		typedef typename OsModel::size_t size_type;
		typedef typename OsModel::block_data_t block_data_t;
		typedef Key_P key_type;
		typedef Key_P value_type;
		typedef Key_P Data;
		...
		public:
			...
			void init(const Data& d) { init(); data_ = d; }
		...
		public:
			...
			///
			const Data& operator*() const { return path_.back()->data(); }
		 ...
		 */
		int init(typename Allocator::self_pointer_t alloc, typename Debug::self_pointer_t debug = 0) {
			allocator_ = alloc;
			#ifdef AVLTREE_DEBUG
				debug_ = debug;
			#endif
		 ...
		 */
		template<typename T>
		inorder_iterator find(const T& data) {
			inorder_iterator iter(allocator_);
			find(data, root(), iter);
			if(iter.path_.size() && iter.path_.back()->cmp(data) == 0) {
				return iter;
			}
			return end();
		}
		 ...
		 */
		const node_ptr_t insert(const Data& data) {
			int height_change = 0;
			node_ptr_t r;
			if(!root_) {
	...
	private:
		 ...
		 */
		node_ptr_t insert(const Data& data, node_ptr_t& base, int& height_change) {
			node_ptr_t r;
