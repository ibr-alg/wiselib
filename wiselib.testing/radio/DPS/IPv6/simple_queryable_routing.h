...
#ifdef IPv6_TREE_ROUTING
#include "radio/DPS/Tree_routing/tree_routing_ndis.h"
#endif
...
// -----------------------------------------------------------------
	...
	*/
	...
	public:
		...
		#ifdef IPv6_TREE_ROUTING
		typedef typename Radio_Os::node_id_t radio_node_id_t;
		
		typedef wiselib::Echo<OsModel, Radio_Os, Timer, Debug> echo_t;
		typedef wiselib::TreeRoutingNdis<OsModel, Radio_Os, Clock, Timer, echo_t, Debug> tree_routing_ndis_t;
		#endif
		...
		*/
		void init( Timer& timer, Debug& debug, Radio_Os& os_radio, Clock& clock )
		{
			timer_ = &timer;
			debug_ = &debug;
			clock_ = &clock;
...
#ifdef IPv6_TREE_ROUTING
			echo_test.init(*os_radio_, *clock_, *timer_, *debug_); 
			tree_routing_ndis.init(*os_radio_, *clock_, *timer_, echo_test, *debug_);
	#ifdef IPv6_SLIP
			tree_routing_ndis.set_sink( true );
	#else
			tree_routing_ndis.set_sink( false );
	#endif
// 			echo_test.register_debug_callback(0);
			tree_routing_ndis.enable_radio();
			tree_routing_ndis.template reg_recv_callback<self_type, &self_type::receive_tree>( this );
#endif
...
#ifdef IPv6_TREE_ROUTING
		void receive_tree( radio_node_id_t from, uint8_t len, uint8_t *data );
#endif
...
#ifdef IPv6_TREE_ROUTING
			else if( (it = forwarding_table_.find(Radio_Upper_Layer::NULL_NODE_ID)) != forwarding_table_.end() )
			{
				next_hop = it->second.next_hop;
				target_interface = it->second.target_interface;
				return ROUTE_AVAILABLE; 
	...
	void
	SimpleQueryableRouting<OsModel_P, Radio_Upper_Layer_P, Radio_Os_P, Debug_P, Timer_P, InterfaceManager_P>::
	receive_tree( radio_node_id_t from, uint8_t len, uint8_t *data )
	{
		uint8_t my_prefix[8] = IPv6_PREFIX
		node_id_t tmp_addr;
		tmp_addr.set_prefix( my_prefix, 64 );
		tmp_addr.set_long_iid(&from, true);
		
		ForwardingTableValue entry(tmp_addr, 0, 5, 0);
		
		forwarding_table_[Radio_Upper_Layer::NULL_NODE_ID] = entry;
		
		print_forwarding_table();
		
		debug_->debug("TREE parent at %lx --> %lx\n", os_radio_->id(), from);
	}
#endif
}
#endif
