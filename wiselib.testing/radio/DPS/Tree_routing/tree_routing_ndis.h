...
namespace wiselib
{
   ...
   private:
      ...
      */
      ...
      {
         TrMsgIdBroadcast = 244, ///< Msg type for broadcasting tree state
         TrMsgIdRouting   = 245  ///< Msg type for routing messages
      };
   ...
   enable_radio( void )
   {
...
#ifdef ROUTING_TREE_DEBUG
      debug().debug( "TreeRouting: Boot for %lx\n", radio().id() );
#endif
      ...
      if ( state_ == TrGateway )
      {
         ...
         hops_ = 0;
#ifdef ROUTING_TREE_DEBUG
         debug().debug( "TreeRouting: Start as sink/gateway, parent = %lx\n", parent_ );
			debug().debug( "TreeTester: NewRoot = %lx\n", parent_ );
#endif
      }
      else
      {
         ...
         debug().debug( "TreeRouting: Start as ordinary node\n" );
			debug().debug( "TreeTester: NewNode = %lx\n", radio().id() ); 
#endif
      }
   ...
   set_sink( bool sink )
   {
      if ( sink ) {
         state_ = TrGateway;
			debug().debug("TreeTester: SetToRoot = %lx\n", radio().id());
		}
      else {
         state_ = TrUnconnected;
			debug().debug("TreeTester: SetToNormalNode = %lx\n", radio().id());
		}
   }
   // -----------------------------------------------------------------------
    template<typename OsModel_P,
   ...
   {	int flags=0;
      ...
      { 
#ifdef ROUTING_TREE_DEBUG
         debug().debug( "TreeRouting: Send to Gate over parent %lx from node %lx ...\n", parent_, radio().id() );
			debug().debug( "TreeTester: SendsOverParent = %lx parent = %lx\n", radio().id(), parent_ );
#endif
      ...
      else
      {
         ...
         debug().debug( "TreeRouting: Not Connected. Cannot send.\n" );
			debug().debug( "TreeTester: CannotSendUncon = %lx\n", radio().id());
#endif
         return ERR_NETDOWN;
      }
   }
   ...
   {
#ifdef ROUTING_TREE_DEBUG
      debug().debug( "TreeRouting: Execute Task 'TreeRouting' at %lx\n", radio().id() );
		debug().debug( "TreeTester: CheckingNeighbors = %lx\n", radio().id());
#endif
...
#ifdef ROUTING_TREE_DEBUG
     //debug().debug( "TreeRouting: Received data %lx length %lx from %lx at %lx\n", (block_data_t)*data, len, from, radio_->id() );
#endif
      ...
      { // debug().debug( "TrMsgIdBroadcast\n");
         ...
         if ( message->hops() + 1 < hops_ )
         {
            ...
            parent_set = true;
            state_ = TrConnected;
            this->notify_receivers( parent_, 0, NULL );
#ifdef ROUTING_TREE_DEBUG
           debug().debug( "TreeRouting:   -> Updated hop count to %i (at node %lx with parent %lx)\n",hops_, radio().id(), parent_ );
			  debug().debug( "TreeTester: NewParentShorterHop = %lx newparent = %lx\n", radio().id(), parent_ );
#endif
         }
      }
...
#ifdef ROUTING_TREE_DEBUG
            debug().debug( "TreeRouting: Routing message at Gate from %lx\n", message->source() );
				debug().debug( "TreeTester: MsgReachedRoot = %lx from = %lx\n", radio().id(), message->source() );
#endif
         }
...
#ifdef ROUTING_TREE_DEBUG
            debug().debug( "TreeRouting: Forward routing message at %lx to %lx\n", radio().id(), parent_ );
				debug().debug( "TreeTester: ForwardOverParent = %lx parent = %lx\n", radio().id(), parent_);
#endif
         }
      }
   }
