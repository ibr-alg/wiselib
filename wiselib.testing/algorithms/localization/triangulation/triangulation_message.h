 ...
 ***************************************************************************/
#ifndef __TRIANGULATION_MSG_H__
#define __TRIANGULATION_MSG_H__
...
namespace wiselib
{
   ...
   public:
     ...
     // bool ack;
      ...
      inline void set_trust(int t)
      { write<OsModel, block_data_t, int>(buffer + TRUST_POS, t); }

      inline int trust()
      { return read<OsModel, block_data_t, int>(buffer + TRUST_POS); }

      inline void set_result(int t)
       { write<OsModel, block_data_t, int>(buffer + RESULT_POS, t); }

       inline int result()
       { return read<OsModel, block_data_t, int>(buffer + RESULT_POS); }

       inline void set_id(node_id_t t)
       { write<OsModel, block_data_t, node_id_t>(buffer + ID_POS, t); }

       inline node_id_t id()
       { return read<OsModel, block_data_t, node_id_t>(buffer + ID_POS); }


      // --------------------------------------------------------------------
            inline Arithmatic distance()
            { return read<OsModel, block_data_t, Arithmatic>(buffer + DISTANCE_POS); }
            // --------------------------------------------------------------------
            inline void set_distance( Arithmatic dist )
            { write<OsModel, block_data_t, Arithmatic>(buffer + DISTANCE_POS, dist); }
   ...
   private:
      enum data_positions
      {
         ...
         TRUST_POS = SEQ_NR_POS + sizeof(seq_nr_t),
         RESULT_POS = TRUST_POS + sizeof(int),
         ID_POS = RESULT_POS + sizeof(int),
         DISTANCE_POS = TRUST_POS + sizeof(int),
         PAYLOAD_POS = DISTANCE_POS + sizeof(Arithmatic)
      };
   ...
   template<typename OsModel_P,
            typename Radio_P,
            typename Arithmatic_P >
   TriangulationMessage<OsModel_P, Radio_P, Arithmatic_P>::
