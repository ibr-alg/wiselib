...
//Metrics for cluster heads
    ...
    init (Radio& tx_radio, Timer& timer, Clock& clock, Debug& debug, Cluster& cluster,  NeighborDiscovery& neighbor) {//HighwayCluster& highwaycluster,
     tx_radio_ = &tx_radio;
     timer_ = &timer;
     clock_ = &clock;
     neighbor_ = &neighbor;
     ...
     cluster_->init( *tx_radio_, *timer_, *debug_, *neighbor_ );
...
template<typename OsModel_P,
    ...
    print_statistics (){
          ...
          debug().debug("RX_total = %i\n; RX_childs = %i\n; FW_childs = %i\n; TX_neigh = %i\n; TX_cluster_head = %i\n",
                    RX_total, RX_childs, FW_childs, TX_neigh, TX_cluster_head);
     }    
}
#endif
