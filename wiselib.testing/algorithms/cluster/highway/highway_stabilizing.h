...
private:
     ...
     inline void set_msg_highway( uint8_t * data, uint8_t msg_id, uint8_t hops, node_id_t candidate_id, node_id_t sid_source, node_id_t sid_target ) {
          memcpy( data, &msg_id, sizeof( uint8_t ) );
          data += sizeof( uint8_t );
          memcpy( data, &hops, sizeof( uint8_t ) );
          data += sizeof( uint8_t );
          memcpy( data, &candidate_id, sizeof( node_id_t ) );
          data += sizeof( node_id_t );
          memcpy( data, &sid_source, sizeof( node_id_t ) );
          data += sizeof( node_id_t );
          memcpy( data, &sid_target, sizeof( node_id_t ) );
          data += sizeof( node_id_t );
     }

     inline void get_msg_highway( msg_highway * msg, uint8_t * data ) {
          memcpy( &( msg->msg_id ), data, sizeof( uint8_t ) );
          data += sizeof( uint8_t );
          memcpy( &( msg->hops ), data, sizeof( uint8_t ) );
          data += sizeof( uint8_t );
          memcpy( &( msg->candidate_id ), data, sizeof( node_id_t ) );
          data += sizeof( node_id_t );
          memcpy( &( msg->sid_source ), data, sizeof( node_id_t ) );
          data += sizeof( node_id_t );
          memcpy( &( msg->sid_target ), data, sizeof( node_id_t ) );
          data += sizeof( node_id_t );
     }
...
template<typename OsModel_P,
         ...
         typename Neighbor_P,
         uint16_t MAX_CLUSTERS>
typename HighwayCluster<OsModel_P, RoutingTable_P, Cluster_P, Neighbor_P, MAX_CLUSTERS>::Node_vect
HighwayCluster<OsModel_P, RoutingTable_P, Cluster_P, Neighbor_P, MAX_CLUSTERS>::
...
template<typename OsModel_P,
         ...
         typename Neighbor_P,
         uint16_t MAX_CLUSTERS>
typename HighwayCluster<OsModel_P, RoutingTable_P, Cluster_P, Neighbor_P, MAX_CLUSTERS>::Ports_vect
HighwayCluster<OsModel_P, RoutingTable_P, Cluster_P, Neighbor_P, MAX_CLUSTERS>::
ports( node_id_t sid )
{
#ifdef HIGHWAY_METHOD_DEBUG
...
#endif
     debug().debug(" Disc foo %d\n", radio().id() );
     if ( pq.size() < MAX_CLUSTER_PORTS )
     {
          pq.push( pin );
          debug().debug(" Disc bar %d\n", radio().id() );
          return;
     }
