...
namespace wiselib
{
	...
	public:
		...
		typedef PLTT_Secure_TraceType<Os, Radio, TimesNumber, SecondsNumber, IntensityNumber, Node, NodeID, Debug> self_type;
		...
		inline PLTT_Secure_TraceType():
		inhibited  ( 0 )
		{}
		inline PLTT_Secure_TraceType(const SecondsNumber& _ds, const IntensityNumber& _da, const IntensityNumber& _sp,	const IntensityNumber& _si, const TimesNumber& _st )
		{
		...
		inline PLTT_Secure_TraceType( const PLTT_TraceData& _td )
		{
			trace_data.current = _td.current;
			trace_data.parent = _td.parent;
			trace_data.grandparent = _td.grandparent;
			for ( uint8_t i=0; i < PRIVACY_CIPHER_TEXT_MAX_SIZE; ++i )
			{
				trace_data.target_id[i] = _td.target_id[i];
			}
			trace_data.start_time = _td.start_time;
			trace_data.diminish_seconds = _td.diminish_seconds;
			trace_data.diminish_amount = _td.diminsh_amount;
			trace_data.spread_penalty = _td.spread_penalty;
			trace_data.intensity = _td.intensity;
			inhibited = 0;
			request_id = 0;
		}
		inline PLTT_Secure_TraceType( const self_type& _t)
		{
			*this = _t;
		}
		inline PLTT_Secure_TraceType( block_data_t* buff, size_t offset = 0 )
		{
		...
		inline block_data_t* set_buffer_from( block_data_t* buff, size_t offset = 0 )
		{
			...
			for ( uint8_t i=0; i < PRIVACY_CIPHER_TEXT_MAX_SIZE; ++i )
			{
				write<Os, block_data_t, block_data_t>( buff + TARGET_ID_POS + i + offset, trace_data.target_id[i] );
			}
		...
		inline void get_from_buffer(block_data_t* buff, size_t offset = 0)
		{
			...
			for ( uint8_t i=0; i < PRIVACY_CIPHER_TEXT_MAX_SIZE; ++i )
			{
				trace_data.target_id[i] = read<Os, block_data_t, block_data_t>( buff + TARGET_ID_POS + i + offset );
			}
		...
		{
			trace_data.current = _t.trace_data.current;
			trace_data.parent = _t.trace_data.parent;
			trace_data.grandparent = _t.trace_data.grandparent;
			for ( uint8_t i=0; i < PRIVACY_CIPHER_TEXT_MAX_SIZE; ++i )
			{
				trace_data.target_id[i] = _t.trace_data.target_id[i];
			}
			trace_data.start_time = _t.trace_data.start_time;
			trace_data.diminish_seconds = _t.trace_data.diminish_seconds;
			trace_data.diminish_amount = _t.trace_data.diminish_amount;
			trace_data.spread_penalty = _t.trace_data.spread_penalty;
			trace_data.intensity = _t.trace_data.intensity;
		...
		inline block_data_t* get_target_id(){ return trace_data.target_id; }
		...
		inline void set_request_id( uint16_t _req_id )
		{
			request_id = _req_id;
		}
		inline void set_target_id( block_data_t* buff, size_t offset = 0 )
		{
			for ( uint8_t i = 0; i < PRIVACY_CIPHER_TEXT_MAX_SIZE; ++i )
			{
				trace_data.target_id[i] = read<Os, block_data_t, block_data_t>( buff + i + offset );
			}
		}
		...
		inline uint8_t compare_target_id( void* node_id )
		{
			for ( uint8_t i = 0; i < PRIVACY_CIPHER_TEXT_MAX_SIZE; ++i )
			{
				if ( trace_data.target_id[i] != *( (block_data_t*)node_id + i ) )
				{
					return 0;
				}
			}
		...
		inline void print( Debug& debug )
		{
			...
			debug.debug( "target id (size %i) : ", 80 );
			for ( uint8_t i = 0; i < PRIVACY_CIPHER_TEXT_MAX_SIZE; i++ )
			{
				debug.debug( " %i", trace_data.target_id[i]);
			}
