...
namespace wiselib
{
   ...
   public:
      ...
      // --------------------------------------------------------------------
      inline uint16_t gate_id()
      { return read<OsModel, block_data_t, uint16_t>(buffer + GATEID_POS); }
      // --------------------------------------------------------------------
      inline void set_gate_id( uint16_t id )
      { write<OsModel, block_data_t, uint16_t>(buffer + GATEID_POS, id); }
      // --------------------------------------------------------------------
   ...
   TreeBroadcastMessage()
   {
      set_msg_id( 0 );
      set_hops( 0 );
      set_gate_id( Radio::NULL_NODE_ID );
   }
   // -----------------------------------------------------------------------
   template<typename OsModel_P,
            typename Radio_P>
