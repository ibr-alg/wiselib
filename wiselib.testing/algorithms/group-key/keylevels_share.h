...
// undef only if using custom made keys! (don't do this :))
#define KEYLEVELS_DEPLOY

#define COMMON_SEED 0
#ifndef SHAWN
  #define KEYSHARE_SIZE 9
  #define KEYPOOL_SIZE 120
#else
...
public:
	...
	typedef typename trusted_links_t::iterator trusted_links_it;
	...
	for(int i=0;i<KEYSHARE_SIZE;i++){
		keyshare[i].key_index = -1;
	}
#endif
	}
	...
	{
		return trusted_links_.begin();
	}

	trusted_links_it tl_end()
	{
		return trusted_links_.end();
	}
...
//		fillKeyshareWithTestKeys(random_);
	...
	void compactListKeyshare(int radio){
                char buf[8*KEYSHARE_SIZE+15];
                int written = sprintf(buf, "GKE_KEY;%x;", radio);

                for (unsigned int i=0; i < KEYSHARE_SIZE; i++)
                {
                        if (i < KEYSHARE_SIZE - 1){
                                written += sprintf(buf + written, "%d;%d;", keyshare[i].key_index, keyshare[i].key_level);
                        }else{
                                written += sprintf(buf + written, "%d;%d", keyshare[i].key_index, keyshare[i].key_level);
                        }
                }       
	...
	{
#ifdef KEYLEVELS_SHARE_DEBUG
		debug_->debug("[KLS] {%d} put_trusted_link to Node %d ",radio_->id(), node);
