        ...
        if(opt_count_)
        {
            ...
            for (opt_index = 0; opt_index < opt_count_; opt_index++)
            {
                ...
                case CONTENT_TYPE:
                    ...
                    break;
                case MAX_AGE:
                    //((coap_packet_t *)packet)->max_age = get_int_opt_value(current_opt, opt_len);
                    break;
                case URI_HOST:
...
protected:
    ...
    uint8_t split_option(uint8_t opt, uint8_t current_delta, uint8_t* buf, char* seperator)
    {
        ...
        while(index < uri_path_len_)
        {
            ...
            buf[last] = (opt - current_delta) << 4 | (index - last - 1);
            memcpy(&buf[last + 1], &uri_path_[last], index - last - 1);
            last = index;
            current_delta = opt;
            opt_count_ += 1;
        }
