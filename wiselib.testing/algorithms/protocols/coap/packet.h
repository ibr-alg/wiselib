...
namespace wiselib {
    ...
    public:
        typedef Coap_P Coap;
        typedef typename Coap::coap_status_t coap_status_t;
#ifdef DEBUG_OPTION
            ...
            if (opt_count_) {
                ...
                for (opt_index = 0; opt_index < opt_count_; opt_index++) {
                    ...
                    switch (current_delta) {
                        case Coap::CONTENT_TYPE:
                            set_option(Coap::CONTENT_TYPE);
                        ...
                        case Coap::BLOCK1:
                            ...
                            set_option(Coap::BLOCK1);
                            break;
                        case Coap::IF_NONE_MATCH:
                            DBG_O(debug().debug("OPTION:IF NONE MATCH"));
                            set_option(Coap::IF_NONE_MATCH);
                            break;
                        default:
                            DBG_O(debug().debug("OPTION:BAD OPTION"));
                            return Coap::BAD_OPTION;
                    }
        ...
        uint8_t packet_to_buffer(uint8_t *buf) {
            ...
            if (is_option(Coap::CONTENT_TYPE)) {
                buf_index += set_int_opt_value(Coap::CONTENT_TYPE, current_delta, &buf[buf_index], content_type_);
                current_delta = Coap::CONTENT_TYPE;
                opt_count_ += 1;
            }
            if (is_option(Coap::MAX_AGE)) {
                buf_index += set_int_opt_value(Coap::MAX_AGE, current_delta, &buf[buf_index], max_age_);
                current_delta = Coap::MAX_AGE;
                opt_count_++;
            }
            if (is_option(Coap::URI_HOST)) {
                buf_index += set_int_opt_value(Coap::URI_HOST, current_delta, &buf[buf_index], uri_host_);
                current_delta = Coap::URI_HOST;
                opt_count_++;
            }
            if (is_option(Coap::URI_PORT)) {
                buf_index += set_int_opt_value(Coap::URI_PORT, current_delta, &buf[buf_index], uri_port_);
                current_delta = Coap::URI_PORT;
                opt_count_++;
            }
            if (is_option(Coap::URI_PATH)) {
                char seperator[] = "/";
                buf_index += split_option(Coap::URI_PATH, current_delta, &buf[buf_index], seperator);
                current_delta = Coap::URI_PATH;
            }
            if (is_option(Coap::OBSERVE)) {
                buf_index += set_int_opt_value(Coap::OBSERVE, current_delta, &buf[buf_index], observe_);
                current_delta = Coap::OBSERVE;
                opt_count_ += 1;
            }
            if (is_option(Coap::TOKEN)) {
                buf[buf_index++] = (Coap::TOKEN - current_delta) << 4 | token_len_;
                memcpy(&buf[buf_index], token_, token_len_);
                current_delta = Coap::TOKEN;
                buf_index += token_len_;
                opt_count_ += 1;
            }
            if (is_option(Coap::ACCEPT)) {
                buf_index += set_int_opt_value(Coap::ACCEPT, current_delta, &buf[buf_index], accept_);
                current_delta = Coap::ACCEPT;
                opt_count_ += 1;
            }
            if (is_option(Coap::BLOCK2)) {
                uint32_t block = block2_num_ << 4;
                if (block2_more_)
                    block |= 0x8;
                block |= 0xF & (power_of_two(block2_size_) - 4);
                buf_index += set_int_opt_value(Coap::BLOCK2, current_delta, &buf[buf_index], block);
                current_delta = Coap::BLOCK2;
                opt_count_ += 1;
            }
            //header
