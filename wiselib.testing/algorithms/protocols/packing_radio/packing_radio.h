		...
		: public RadioBase<OsModel_P, typename Radio_P::node_id_t, typename Radio_P::size_t, typename Radio_P::block_data_t>
	{
		public:
			...
			void flush() {
				if(!buffer_virgin()) {
					...
					debug_->debug("prad fsh snd don");

					packer_.clear();
					debug_->debug("prad fsh clr don");
				}
			}
		...
		private:
			...
			void on_receive(node_id_t from, size_t size, block_data_t* data) {
				debug_->debug("@%d prad recv", (int)radio_->id());
				...
				while(true) {
					...
					debug_->debug("prad rcv loop don");
					//DBG("unpkg don");
